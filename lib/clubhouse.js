  ...
  return {
    ...
    getStories: async function(){
      ...
      for(let index = 0; index < stories.length; index++){
        ...
        story['createdAt'] = await clubhouseHelper.getCreatedAt(oneStory)
        story['completedAt'] = clubhouseHelper.getCompletedAt(oneStory)
        var createdAtTimestamp = Date.parse(story['createdAt'])
        var completedAtTimestamp = Date.parse(story['completedAt'])
        var day = 1000 * 24 * 60 * 60.0
        story['leadTime'] = (completedAtTimestamp - createdAtTimestamp) / day
    ...
    getCompletedAt: function(story){
      return story.find('.completed-column').attr('data-tooltip')
    },

    getCreatedAt: async function(story){
      story.find('.name-column a')[0].click()
      var createdAt = ''
      await new Promise((resolve) => setTimeout(resolve, 100))
      createdAt = $('.modal-dialog.modal-dialog-fullscreen.element-is-open').find('.attribute.story-date-created .value').attr('data-tooltip')
      $('.modal-dialog.modal-dialog-fullscreen.element-is-open').find('a.close-button').click()
      return createdAt
    }
